  ...
  private buildTools(): AnyAgentTool[] {
    ...
    return [
      ...
      },
      {
        ...
        parameters: Type.Object({
          ...
          accountId: Type.Optional(
            Type.String({
              description: "Optional override for the workspace owner. Normally the tool resolves the workspace by scanning /workspace/*/<filePath> for a unique match — pass this only when you have multiple workspaces with the same relative path and need to disambiguate (rare).",
            }),
          ),
        }),
        async execute(_id: string, params: Record<string, unknown>) {
          ...
          // Resolve which `/workspace/<accountId>/` to read from. Three
          // strategies, in priority order:
          //
          //   1. Explicit `accountId` param — if the model passes it, trust it.
          //   2. `OPENCLAW_ACCOUNT_ID` env var — set by the gateway runtime
          //      when it's wired to do so. As of 2026-05-07 NOTHING in the
          //      openclaw codebase actually sets this var (verified via
          //      `rg -F OPENCLAW_ACCOUNT_ID` on the gateway repo) so it
          //      falls through to (3) in practice.
          //   3. Workspace scan — look for `/workspace/*/<filePath>` and use
          //      the unique match. This is the fallback that lets
          //      Nova/Pixel/Theo etc. attach files written to their own
          //      workspace without the runtime needing to inject env.
          //
          // Strategy 3 is heuristic but safe: the upload itself is gated by
          // the runtime token (the backend validates the agent's identity
          // and pod-membership), so even if a scan picks the wrong
          // workspace the upload still couldn't post into a pod the caller
          // doesn't belong to. Worst case is a confusing error if two
          // workspaces happen to have the same relative path.
          const explicitAccountId = readStringParam(params, "accountId");
          const envAccountId = (process.env.OPENCLAW_ACCOUNT_ID || "").trim();
          // eslint-disable-next-line @typescript-eslint/no-require-imports
          const fs = require("node:fs");
          // eslint-disable-next-line @typescript-eslint/no-require-imports
          const pathMod = require("node:path");

          const validateRelative = (rel: string): string => {
            // Require relative + no escape attempts.
            if (rel.startsWith("/") || rel.includes("..")) {
              throw new Error(
                `commonly_attach_file: filePath must be a relative path inside the workspace; got '${rel}'`,
              );
            }
            return rel.replace(/^\.\//, "");
          };

          const safeRelative = validateRelative(filePath);

          let workspaceRoot: string | null = null;
          let absolutePath: string | null = null;

          // (1) explicit param + (2) env var
          for (const candidate of [explicitAccountId, envAccountId]) {
            if (!candidate) continue;
            const root = `/workspace/${candidate}`;
            const abs = pathMod.join(root, safeRelative);
            try {
              fs.accessSync(abs, constants.R_OK);
              workspaceRoot = root;
              absolutePath = abs;
              break;
          ...
          // (3) scan all /workspace/*/ — match exactly one
          if (!absolutePath) {
            let entries: string[] = [];
            try {
            ...
            const matches: { root: string; abs: string }[] = [];
            for (const name of entries) {
              const root = `/workspace/${name}`;
              const abs = pathMod.join(root, safeRelative);
              try {
                fs.accessSync(abs, constants.R_OK);
                matches.push({ root, abs });
              } catch { /* not in this one */ }
            }
            if (matches.length === 1) {
              workspaceRoot = matches[0].root;
              absolutePath = matches[0].abs;
            } else if (matches.length > 1) {
              const which = matches.map((m) => m.root).join(", ");
              throw new Error(
                `commonly_attach_file: ambiguous filePath '${filePath}' — found in multiple workspaces (${which}). Pass accountId param or use a path unique to your workspace.`,
              );
            }
          }
          ...
          if (!absolutePath || !workspaceRoot) {
            throw new Error(
              `commonly_attach_file: cannot find '${filePath}' under any /workspace/<accountId>/. ` +
              `Check the file exists and the path is relative to your workspace root (e.g. 'out/report.docx', not '/workspace/nova/out/report.docx').`,
            );
          }
          ...
          try {
            ...
            throw new Error(
              `commonly_attach_file: cannot read file at '${absolutePath}' — ${(err as Error).message}`,
            );
          }
      ...
      },
      {
        ...
        description:
          "Read a file attached in chat (the `fileName` from a [[upload:fileName|originalName|size|kind|id]] directive) and return its text content. " +
          "Use when a user / teammate attaches a file you need to consume (briefs, specs, decks, datasets). " +
          "Format-aware extraction: .docx/.xlsx/.pptx via `officecli view text`; .pdf via `pdftotext`; .md/.txt/.csv/.json/.html as raw text; everything else falls back to `markitdown`. " +
          "Returns up to maxChars characters of extracted text (default 16000, max 64000) — truncates with a clear marker if longer. " +
          "Requires the file to be in object storage (i.e. uploaded via the upload route). Eliminates the multi-step curl + extract dance you'd otherwise have to write inline.",
        parameters: Type.Object({
          fileName: Type.String({
            description:
              "ObjectStore key from the upload directive's first pipe-separated field (e.g. '1778103164116-632900841.docx'). NOT the human originalName — pass the literal fileName as it appears between `[[upload:` and the first `|`.",
          }),
          maxChars: Type.Optional(
            Type.Number({
              description: "Maximum characters of extracted text to return (default 16000, max 64000). Truncated output ends with a `[…truncated, N more chars]` marker.",
            }),
          ),
        }),
        async execute(_id: string, params: Record<string, unknown>) {
          ...
          const maxChars = Math.min(Math.max(requestedCap || 16000, 1000), 64000);

          // Defense: only allow what looks like a real ObjectStore key
          // (`<digits>-<digits>.<ext>`) so callers can't pass `../../etc/passwd`
          // or arbitrary URL fragments. Backend would 404 anyway, but failing
          // fast keeps the error message actionable.
          if (!/^[a-zA-Z0-9_.-]+\.[a-zA-Z0-9]+$/.test(fileName) || fileName.includes("..")) {
            throw new Error(
              `commonly_read_attachment: invalid fileName '${fileName}' — expected the ObjectStore key from an [[upload:...]] directive`,
            );
          }

          const baseUrl = process.env.COMMONLY_API_URL || "";
          const token = process.env.COMMONLY_API_TOKEN || "";
          if (!baseUrl) {
            throw new Error("commonly_read_attachment: COMMONLY_API_URL not configured");
          }

          // Fetch the attachment. Public-read works today (ADR-002 Phase 1);
          // sending the runtime token is defensive against the upcoming
          // signed-URL flip — when reads require auth, this still works.
          const url = `${baseUrl}/api/uploads/${encodeURIComponent(fileName)}`;
          let res: Response;
          try {
            res = await fetch(url, {
            ...
            throw new Error(
              `commonly_read_attachment: fetch failed — ${(err as Error).message}`,
            );
          }
          if (!res.ok) {
            throw new Error(
              `commonly_read_attachment: server returned ${res.status} for '${fileName}'`,
            );
          }
          const ab = await res.arrayBuffer();
          const bytes = Buffer.from(ab);
          // Hard cap at 25 MB to mirror the upload route — bigger uploads
          // shouldn't exist, but if they do we don't want to OOM the gateway.
          ...
          // Extract: write the bytes to a temp file (most extractors are
          // file-based) and shell out. We deliberately don't try every
          // possible extractor — a small switch on extension keeps the
          // failure mode debuggable.
          const ext = (fileName.split(".").pop() || "").toLowerCase();
          const dir = mkdtempSync(joinPath(tmpdir(), "commonly-read-"));
          const localPath = joinPath(dir, fileName);
          let extracted = "";
          let extractor = "raw";
          try {
            writeFileSync(localPath, bytes);
            const runExtractor = (cmd: string, args: string[]): Promise<string> =>
              new Promise((resolve, reject) => {
                ...
                child.on("close", (code) => {
                  ...
                  else reject(new Error(`${cmd} exited ${code}: ${errOut.slice(0, 400)}`));
                });
              });
            ...
            if (ext === "docx" || ext === "xlsx" || ext === "pptx") {
              ...
              ext === "md" || ext === "txt" || ext === "csv" || ext === "json" ||
              ext === "log" || ext === "yaml" || ext === "yml" || ext === "xml" ||
              ext === "html" || ext === "htm"
            ) {
              extractor = "raw";
              extracted = bytes.toString("utf8");
            } else {
              // Fallback: try markitdown — covers epub, audio transcripts,
              // and oddball formats. If markitdown also fails the caller
              // gets a clean error.
